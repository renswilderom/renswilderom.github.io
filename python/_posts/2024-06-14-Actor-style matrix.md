...
Imagine a horror film director who pioneers the idea of producing a "zombie" movie. The title captures the public's imagination and becomes a hit, prompting other directors to follow suit. Soon, the market is flooded with zombie films. However, over time, audiences grow weary of the same brain-dead walkers, and interest wanes. The market for this particular style becomes saturated, leading to a decline in production. As audiences seek something fresh, a new stylistic trend has the opportunity to emerge.

In the context of 'cultural endogenous' dynamics (Van Venrooij, 2015; Godart and Galunic, 2019; Sgourev, Aadland, and Formilan, 2023), it is interesting to ask: How does the alignment between audiences and film styles evolve over time? How do filmmakers adapt to styles that fall in and out of fashion? Understanding these dynamics could offer valuable insights into how the evolution of style influences markets for creative works, such as the film industry, and shed light on the broader constitutive effects of culture.

To identify distinct styles in films, I use BERTopic (see Grootendorst, 2022, and [BERTopic's homepage](https://maartengr.github.io/BERTopic/index.html){:target="_blank"}) to cluster frequently co-occurring film keywords into topics. The main contribution of this blog post lies in Sections 3 and 4, where I build on the hierarchical cluster matrix generated by BERT to compute style clusters, generate time series data for these clusters, and link them to the individuals involved in their production. The code can be tested using a small dataset of 3,000 horror films released in the US, which includes both plot-related keywords and information about their creators, as detailed in the next section.
...
With the time series-style data prepared, we can now connect each film's core crew members to the given styles. The final code cell uses `groupby` and `pivot` to generate a dataframe, allowing us to track which filmmakers are linked to particular style clusters in any given year.
